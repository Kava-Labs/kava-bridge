...
describe("WKAVA", function () {
  ...
  describe("deposit", function () {
    it("should allow users to deposit", async function () {
        const userBalanceBefore = await wkava.balanceOf(await addr2.getAddress());
        expect(userBalanceBefore).to.equal(0);

        const depositAmt = 5000;
        await wkava.connect(addr2).deposit({ value: depositAmt });

        const userBalanceAfter = await wkava.balanceOf(await addr2.getAddress());
    ...
    it("should emit Deposit event", async function () {
        const depositAmt = 5000;
        const tx: ContractTransaction = await wkava.connect(addr2).deposit({ value: depositAmt });
        ...
        expect(event?.args?.src).to.equal(await addr2.getAddress());
  ...
  describe("withdraw", function () {
    ...
    it("should allow users to withdraw", async function () {
        ...
        await wkava.connect(addr2).withdraw(withdrawAmt);

        const userBalanceAfter = await wkava.balanceOf(await addr2.getAddress());
        expect(userBalanceAfter).to.equal(depositAmt-withdrawAmt);

        await wkava.connect(addr2).withdraw(withdrawAmt);
        const userBalanceFinal = await wkava.balanceOf(await addr2.getAddress());
        expect(userBalanceFinal).to.equal(0);
    });

    it("should emit Withdrawal event", async function () {
        const tx: ContractTransaction = await wkava.connect(addr2).withdraw(depositAmt);
        let receipt: ContractReceipt = await tx.wait();
